...
public class TestExecuteDaiMojoScoringPipeline {

	private TestRunner runner;
	private ExecuteDaiMojoScoringPipeline processor;
	...
	public void setup() throws InitializationException, IOException {
		processor = new ExecuteDaiMojoScoringPipeline();
		...
		runner.setProperty(ExecuteDaiMojoScoringPipeline.RECORD_READER, "reader");
		
		writerService = new CSVRecordSetWriter();

		runner.addControllerService("writer", writerService);

		runner.setProperty(ExecuteDaiMojoScoringPipeline.RECORD_WRITER, "writer");
	...
	public void testRelationshipsCreated() throws IOException {
		generateDefaultHydraulicTestData(1);
		...
		final String pipelineMojoPath = System.getProperty("user.home") + "/daimojo-nifi/mojo-pipeline/pipeline.mojo";
		
		runner.setProperty(ExecuteDaiMojoScoringPipeline.PIPELINE_MOJO_FILEPATH, pipelineMojoPath);
		runner.enqueue(new byte[0]);
		
		Set<Relationship> relationships = processor.getRelationships();
		assertTrue(relationships.contains(ExecuteDaiMojoScoringPipeline.REL_FAILURE));
		assertTrue(relationships.contains(ExecuteDaiMojoScoringPipeline.REL_SUCCESS));
		assertTrue(relationships.contains(ExecuteDaiMojoScoringPipeline.REL_ORIGINAL));
	...
	public void testNoFlowFileContent() throws IOException {
		...
		runner.assertTransferCount(ExecuteDaiMojoScoringPipeline.REL_FAILURE, 0);
		runner.assertTransferCount(ExecuteDaiMojoScoringPipeline.REL_SUCCESS, 0);
	}
	
	@Test
	public void testNoRecords() throws IOException {
		generateDefaultHydraulicTestData(0);
		// "Inherit Record Schema" is set by default for CSVRecordSetWriter SCHEMA_ACCESS_STRATEGY property, so it commented out
		// runner.setProperty(writerService, SchemaAccessUtils.SCHEMA_ACCESS_STRATEGY, SchemaAccessUtils.INHERIT_RECORD_SCHEMA);
		runner.enableControllerService(writerService);
		...
		runner.assertQueueEmpty();
		runner.assertTransferCount(ExecuteDaiMojoScoringPipeline.REL_FAILURE, 0);
		runner.assertTransferCount(ExecuteDaiMojoScoringPipeline.REL_SUCCESS, 1);
		runner.assertTransferCount(ExecuteDaiMojoScoringPipeline.REL_ORIGINAL, 1);
	}
	...
	public void testInvalidFlowFileContent() throws IOException {
		...
		runner.assertAllFlowFilesTransferred(ExecuteDaiMojoScoringPipeline.REL_FAILURE);
	}
	...
	private void generateDefaultHydraulicTestData(int numRecords) {
		generateHydraulicTestData(numRecords, null);
	}
